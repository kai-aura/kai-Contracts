 ...
 */
contract KaiLocker is ReentrancyGuard, Ownable, IAuraLocker {
    ...
    //     Duration that rewards are streamed over. 12 days.
    uint256 public constant rewardsDuration = 86400 * 12;
    //     Duration of lock/earned penalty period. 28 days.
    uint256 public constant lockDuration = 86400 * 28;
    ...
    ****************************************/
     ...
     */
    constructor(string memory _nameArg, string memory _symbolArg, address _stakingToken, address _cvxCrv) Ownable() {
        _name = _nameArg;
        _symbol = _symbolArg;
        _decimals = 18;
    ...
    ****************************************/
    ...
    function _lock(address _account, uint256 _amount) internal notBlacklisted(msg.sender, _account) {
        ...
        if (delegatee != address(0)) {
            delegateeUnlocks[delegatee][unlockTime] = delegateeUnlocks[delegatee][unlockTime].add(lockAmount);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        lockedSupply = lockedSupply.sub(amt);
    ...
    function _processExpiredLocks(
        ...
        if (isShutdown || locks[length - 1].unlockTime <= expiryTime) {
            ...
            for (uint256 i = nextUnlockIndex; i < length; i = unchkIncr(i)) {
    ...
    ****************************************/
    ...
    function delegate(address newDelegatee) external virtual nonReentrant {
        ...
        while (currentLock.unlockTime > upcomingEpoch) {
            futureUnlocksSum = futureUnlocksSum.add(currentLock.amount);

            if (oldDelegatee != address(0)) {
                delegateeUnlocks[oldDelegatee][currentLock.unlockTime] = delegateeUnlocks[oldDelegatee][
                    currentLock.unlockTime
                ].sub(currentLock.amount);
            }

            delegateeUnlocks[newDelegatee][currentLock.unlockTime] = delegateeUnlocks[newDelegatee][
                currentLock.unlockTime
            ].add(currentLock.amount);
            ...
            if (ckpts.length > 0) {
                ...
                // e.g. week 1 + 17 <= 18
                    ...
                    while (nextEpoch > prevCkpt.epochStart) {
                        unlocksSinceLatestCkpt = unlocksSinceLatestCkpt.add(delegateeUnlocks[_account][nextEpoch]);
                        nextEpoch = nextEpoch.sub(rewardsDuration);
                    }
                    ckpts.push(
                        DelegateeCheckpoint({
    ...
    function getPastVotes(address account, uint256 timestamp) public view returns (uint256 votes) {
        ...
        while (epoch > ckpt.epochStart) {
            votes = votes.sub(delegateeUnlocks[account][epoch]);
            epoch = epoch.sub(rewardsDuration);
        }
    }
    ...
    function _checkpointsLookup(
        ...
        uint256 epochStart
    ) internal view returns (DelegateeCheckpoint memory) {
    ...
    ****************************************/
    ...
    function lockedBalances(
        ...
        uint256 idx;
        for (uint256 i = nextUnlockIndex; i < locks.length; i = unchkIncr(i)) {
    ...
    ****************************************/
    ...
    function queueNewRewards(address _rewardsToken, uint256 _rewards) external nonReentrant {
        require(rewardDistributors[_rewardsToken][msg.sender] || msg.sender == owner(), "!authorized");
